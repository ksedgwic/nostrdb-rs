...
mod tests {
    ...
    async fn multiple_events_work() {
...

        {
            ...
            let sub = ndb.subscribe(&[filter]).expect("sub_id");
            let waiter = ndb.wait_for_notes(sub, 10);
            ...
            let res = waiter.await.expect("await ok");
            assert_eq!(res.len(), 6); // often fails, but not always ...
        }
    }
    ...
    async fn multiple_events_with_final_pause_work() {
...

        {
            ...
            let res = time::timeout(timeout_duration, waiter).await;
            match res {
                Ok(Ok(result)) => {
                    assert_eq!(result.len(), 6);
                }
                Ok(Err(err)) => panic!("Error waiting for notes: {:?}", err),
                Err(_) => panic!("Timed out waiting for notes"),
            }
        }
    }
